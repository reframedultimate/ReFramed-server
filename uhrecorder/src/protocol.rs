...
pub fn send_constants(client: &mut TcpStream) -> Result<(), std::io::Error> {
...
pub fn send_fighter_info(server: &Server, frame: u32, entry_id: i32, stock_count: u8, status_kind: i32, damage: f32) {
    ...
    server.broadcast(&[
        ...
        entry_id as u8,
        stock_count,
        ((status_kind >> 8) & 0xFF) as u8,
        ((status_kind >> 0) & 0xFF) as u8,
        ((damage_int >> 8) & 0xFF) as u8,
        ((damage_int >> 0) & 0xFF) as u8
    ]);
}
...
fn send_stage_constants(client: &mut TcpStream) -> Result<(), std::io::Error> {
    for (kind, name) in [
        ...
        let kind_h = ((*kind >> 0) & 0xFF) as u8;
        let kind_l = ((*kind >> 8) & 0xFF) as u8;
        let name_bytes = name.as_bytes();
        let data = &[MessageType::StageKinds.into(), kind_h, kind_l, name_bytes.len() as u8];
        let data = &[data, name_bytes].concat();
        client.write(data)?;
    }
    Ok(())
